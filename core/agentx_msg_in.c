...
var_bind_alloc(uint8_t **buffer, uint8_t flag, enum agentx_err_code *err)
{
  ...
  /* value type */
  type = htoax16(*(uint16_t *)buf, flag);
  buf += 4;
...
search_range_alloc(uint8_t **buffer, uint8_t flag, enum agentx_err_code *err)
{
  ...
  if (start_len / sizeof(uint32_t) > ASN1_OID_MAX_LEN) {
  ...
  if (end_len / sizeof(uint32_t) > ASN1_OID_MAX_LEN) {
    *err = AGENTX_ERR_SR_OID_LEN;
    return NULL;
  }
  end_include = *(buf + 2);
...
pdu_hdr_parse(struct agentx_datagram *xdg, uint8_t **buffer)
{
  ...
  flags = xdg->pdu_hdr.flags;

  xdg->pdu_hdr.session_id = axtoh32(*(uint32_t *)buf, flags);
  buf += sizeof(uint32_t);
  xdg->pdu_hdr.transaction_id = axtoh32(*(uint32_t *)buf, flags);
  buf += sizeof(uint32_t);
  xdg->pdu_hdr.packet_id = axtoh32(*(uint32_t *)buf, flags);
  buf += sizeof(uint32_t);
  xdg->pdu_hdr.payload_length = axtoh32(*(uint32_t *)buf, flags);
  buf += sizeof(uint32_t);

  /* Optinal context */
  if (xdg->pdu_hdr.flags & NON_DEFAULT_CONTEXT) {
    xdg->ctx_len = axtoh32(*(uint32_t *)buf, flags);
  ...
  case AGENTX_PDU_GETBULK:
    xdg->u.getbulk.non_rep = axtoh16(*(uint16_t *)buf, flags);
    buf += sizeof(uint16_t);
    xdg->u.getbulk.max_rep = axtoh16(*(uint16_t *)buf, flags);
    buf += sizeof(uint16_t);
    xdg->pdu_hdr.payload_length -= 2 * sizeof(uint16_t);
    break;
  case AGENTX_PDU_RESPONSE:
    xdg->u.response.sys_up_time = axtoh32(*(uint32_t *)buf, flags);
    buf += sizeof(uint32_t);
    xdg->u.response.error = axtoh16(*(uint16_t *)buf, flags);
    buf += sizeof(uint16_t);
    xdg->u.response.index = axtoh16(*(uint16_t *)buf, flags);
    buf += sizeof(uint16_t);
